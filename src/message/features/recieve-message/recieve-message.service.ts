...
export class RecieveMessageService
  implements CommandHandler<RecieveMessageCommand, Message>
{
  constructor(
  ...
  async execute(request: RecieveMessageCommand): Promise<Message> {
    this.logger.debug(request);

    const message = await this.messageRepository.findById(request.messageId);

    if (!message) {
      throw new MessageNotFoundError(request.messageId);
    }
    ...
    this.logger.log(`Delivered message ${savedMessage.id}`);
