...
@Service()
export class LocalThreadRepository
	implements ThreadRepository
{
	...
	async save(thread: Thread) {
		...
		const existingThread = THREAD_STORE.find(
			(t) => t.id === thread.id,
		)
	...
	async findByMembers(members: User[]) {
		...
		const thread = THREAD_STORE.find((t) => {
			const threadMemberIds = t.members.map(
				(u) => u.id,
			)
			return memberIds.every((id) =>
				threadMemberIds.includes(id),
			)
		})
		...
		const messagesOfThread =
			await this.messageRepository.findByThread(
				thread,
			)

		return new Thread(
			{ members, messages: messagesOfThread },
			thread.id,
		)
	}
	...
	async findById(threadId: string) {
		const thread = THREAD_STORE.find(
			(t) => t.id === threadId,
		)
